...
const QuantityGroupPage = () => {
    const [selectedGroup, setSelectedGroup] = useState(null);
    const [startDate, setStartDate] = useState(null);
    const [endDate, setEndDate] = useState(null);

    const [isAdminViewer, setIsAdminViewer] = useState(false);
    const { data, error, loading } = useGetAllSiteGroupQuery();
    const [
        getQuantityCompany,
    ...
    if (data != null && data != undefined) {
        if (data.GetAllSiteGroup != null && data.GetAllSiteGroup != undefined) {
            if (data.GetAllSiteGroup.length > 0) {
                for (let group of data.GetAllSiteGroup) {
                    let obj: Companies = {
                        // @ts-ignore comment
                        value: group?.Group,
                        // @ts-ignore comment
    ...
    const renderTableQuantity = (data: any) => {
        let sortedData = quickSort(data.QuantityDayGroup);
...
export default QuantityGroupPage;
