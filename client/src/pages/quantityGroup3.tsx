...
const QuantityGroup3Page = () => {
    const [selectedGroup, setSelectedGroup] = useState(null);
    const [startDate, setStartDate] = useState(null);
    const [endDate, setEndDate] = useState(null);

    const [isAdminViewer, setIsAdminViewer] = useState(false);
    const { data, error, loading } = useGetAllSiteGroup3SQuery();
    const [
        getQuantityCompany,
    ...
    if (data != null && data != undefined) {
        if (
            data.GetAllSiteGroup3S != null &&
            data.GetAllSiteGroup3S != undefined
        ) {
            if (data.GetAllSiteGroup3S.length > 0) {
                for (let group of data.GetAllSiteGroup3S) {
                    let obj: Companies = {
                        // @ts-ignore comment
                        value: group?.Group,
                        // @ts-ignore comment
    ...
    const renderTableQuantity = (data: any) => {
        let sortedData = quickSort(data.QuantityDayGroup3);
                                ...
                                <>
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            exportTableToExcel(
                                                'tableQuantity',
                                                `Sản lượng ${selectedGroup} từ ${convertDateToStringNotTimeForTitle(
                                                    // @ts-ignore comment
                                                    new Date(startDate),
...
export default QuantityGroup3Page;
