...
const QuantityGroup2Page = () => {
    const [selectedGroup, setSelectedGroup] = useState(null);
    const [startDate, setStartDate] = useState(null);
    const [endDate, setEndDate] = useState(null);

    const [isAdminViewer, setIsAdminViewer] = useState(false);
    const { data, error, loading } = useGetAllSiteGroup2SQuery();
    const [
        getQuantityCompany,
    ...
    if (data != null && data != undefined) {
        if (
            data.GetAllSiteGroup2S != null &&
            data.GetAllSiteGroup2S != undefined
        ) {
            if (data.GetAllSiteGroup2S.length > 0) {
                for (let group of data.GetAllSiteGroup2S) {
                    let obj: Companies = {
                        // @ts-ignore comment
                        value: group?.Group,
                        // @ts-ignore comment
    ...
    const renderTableQuantity = (data: any) => {
        let sortedData = quickSort(data.QuantityDayGroup2);
...
export default QuantityGroup2Page;
