...
const QuantityGroup4Page = () => {
    const [selectedGroup, setSelectedGroup] = useState(null);
    const [startDate, setStartDate] = useState(null);
    const [endDate, setEndDate] = useState(null);

    const [isAdminViewer, setIsAdminViewer] = useState(false);
    const { data, error, loading } = useGetAllSiteGroup4SQuery();
    const [
        getQuantityCompany,
    ...
    if (data != null && data != undefined) {
        if (
            data.GetAllSiteGroup4S != null &&
            data.GetAllSiteGroup4S != undefined
        ) {
            if (data.GetAllSiteGroup4S.length > 0) {
                for (let group of data.GetAllSiteGroup4S) {
                    let obj: Companies = {
                        // @ts-ignore comment
                        value: group?.Group,
                        // @ts-ignore comment
    ...
    const renderTableQuantity = (data: any) => {
        let sortedData = quickSort(data.QuantityDayGroup4);
...
export default QuantityGroup4Page;
