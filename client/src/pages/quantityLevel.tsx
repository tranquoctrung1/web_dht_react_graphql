...
const QuantityLevelPage = () => {
    const [selectedLevel, setSelectedLevel] = useState(null);
    const [startDate, setStartDate] = useState(null);
    const [endDate, setEndDate] = useState(null);

    const [isAdminViewer, setIsAdminViewer] = useState(false);
    const { data, error, loading } = useGetAllSiteLevelQuery();
    const [
        getQuantityCompany,
    ...
    if (data != null && data != undefined) {
        if (data.GetAllSiteLevel != null && data.GetAllSiteLevel != undefined) {
            if (data.GetAllSiteLevel.length > 0) {
                for (let level of data.GetAllSiteLevel) {
                    let obj: Companies = {
                        // @ts-ignore comment
                        value: level?.Level,
                        // @ts-ignore comment
    ...
    const renderTableQuantity = (data: any) => {
        let sortedData = quickSort(data.QuantityDayLevel);
                            ...
                            {dataQuanity && (
                                <>
                                    ...
                                    <ReactHTMLTableToExcel
                                        ...
                                        table="tableQuantity"
                                        filename={`Sản lượng ${selectedLevel} từ ${convertDateToStringNotTimeForTitle(
                                            // @ts-ignore comment
                                            new Date(startDate),
...
export default QuantityLevelPage;
