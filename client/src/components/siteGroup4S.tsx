...
const SiteGroup4S = () => {
    ...
    useEffect(() => {
        getSiteGroup()
            .then((res) => {
                if (res?.data?.GetAllSiteGroup4S) {
                    const temp = [];
                    for (const item of res.data.GetAllSiteGroup4S) {
                        const obj = {
                            value: item?.Group,
                    ...
                    //@ts-ignore
                    setSiteGroup([...res.data.GetAllSiteGroup4S]);
                }
            })
            .catch((err) => {
                console.error(err);
                ...
                .then((res) => {
                    if (res?.data?.InsertSiteGroup4S) {
                        if (res.data?.InsertSiteGroup4S !== '') {
                            ...
                            setValue('_id', res.data.InsertSiteGroup4S);
    ...
    const onUpdateClicked = () => {
        ...
        if (isAllow) {
            updateSiteGroup({
                ...
                .then((res) => {
                    if (res?.data?.UpdateSiteGroup4S) {
                        if (res.data.UpdateSiteGroup4S > 0) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Successfull',
            ...
            if (result.isConfirmed) {
                ...
                if (isAllow) {
                    deleteSiteGroup({
                        ...
                        .then((res) => {
                            if (res?.data?.DeleteSiteGroup4S) {
                                if (res.data.DeleteSiteGroup4S > 0) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Successfull',
