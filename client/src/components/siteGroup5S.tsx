...
import {
    useGetAllSiteGroup5SLazyQuery,
    useInsertSiteGroup5SMutation,
    useUpdateSiteGroup5SMutation,
    useDeleteSiteGroup5SMutation,
...
const SiteGroup5S = () => {
    ...
    const [getSiteGroup] = useGetAllSiteGroup5SLazyQuery();
    const [insertSiteGroup] = useInsertSiteGroup5SMutation();
    const [updateSiteGroup] = useUpdateSiteGroup5SMutation();
    const [deleteSiteGroup] = useDeleteSiteGroup5SMutation();
    ...
    useEffect(() => {
        getSiteGroup()
            .then((res) => {
                if (res?.data?.GetAllSiteGroup5S) {
                    const temp = [];
                    for (const item of res.data.GetAllSiteGroup5S) {
                        const obj = {
                            value: item?.Group,
                    ...
                    //@ts-ignore
                    setSiteGroup([...res.data.GetAllSiteGroup5S]);
                }
            })
            .catch((err) => {
                console.error(err);
                ...
                .then((res) => {
                    if (res?.data?.InsertSiteGroup5S) {
                        if (res.data?.InsertSiteGroup5S !== '') {
                            ...
                            setValue('_id', res.data.InsertSiteGroup5S);
    ...
    const onUpdateClicked = () => {
        ...
        if (isAllow) {
            updateSiteGroup({
                ...
                .then((res) => {
                    if (res?.data?.UpdateSiteGroup5S) {
                        if (res.data.UpdateSiteGroup5S > 0) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Successfull',
            ...
            if (result.isConfirmed) {
                ...
                if (isAllow) {
                    deleteSiteGroup({
                        ...
                        .then((res) => {
                            if (res?.data?.DeleteSiteGroup5S) {
                                if (res.data.DeleteSiteGroup5S > 0) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Successfull',
