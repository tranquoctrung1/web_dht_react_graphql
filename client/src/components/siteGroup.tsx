...
const SiteGroup = () => {
    ...
    const [insertSiteGroup] = useInsertSiteGroupMutation();
    const [updateSiteGroup] = useUpdateSiteGroupMutation();
    const [deleteSiteGroup] = useDeleteSiteGroupMutation();
        ...
        getSiteGroup()
            .then((res) => {
                if (res?.data?.GetAllSiteGroup) {
                    ...
                    //@ts-ignore
                    setSiteGroup([...res.data.GetAllSiteGroup]);
                }
            })
            .catch((err) => {
                console.error(err);
                ...
                .then((res) => {
                    if (res?.data?.InsertSiteGroup) {
                        if (res.data?.InsertSiteGroup !== '') {
                            ...
                            setValue('_id', res.data.InsertSiteGroup);
    ...
    const onUpdateClicked = () => {
        ...
        if (isAllow) {
            updateSiteGroup({
                ...
                .then((res) => {
                    if (res?.data?.UpdateSiteGroup) {
                        if (res.data.UpdateSiteGroup > 0) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Successfull',
            ...
            if (result.isConfirmed) {
                ...
                if (isAllow) {
                    deleteSiteGroup({
                        ...
                        .then((res) => {
                            if (res?.data?.DeleteSiteGroup) {
                                if (res.data.DeleteSiteGroup > 0) {
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Successfull',
